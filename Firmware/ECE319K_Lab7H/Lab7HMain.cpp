...
SlidePot Sensor(1786, 159); // needs calibration
...
void OutFix(uint32_t n) {
  ...
  char result[10];
  uint32_t i = 0;
  uint32_t intChar = n / 1000;
  uint32_t fracChar = n % 1000;

  result[i++] = (intChar + '0');
  result[i++] = '.';
  result[i++] = (fracChar / 100) + '0';
  fracChar %= 100;
  result[i++] = (fracChar / 10) + '0';
  fracChar %= 10;
  result[i++] = (fracChar) + '0';
  ...
  ST7735_DrawString(0, 0, result, ST7735_MAGENTA);
}
...
uint32_t startTime, stopTime, startTime2, stopTime2;
uint32_t Offset, ADCtime, Converttime, FloatConverttime, OutFixtime,
    FloatOutFixtime, TotalTime; // in bus cycles
uint32_t Time;
...
int main3(void) { // main3
  ...
  while (1) {
    ...
    stopTime = SysTick->VAL;
    Converttime =
        ((startTime - stopTime) & 0x0FFFFFF) - Offset; // in bus cycles
    startTime = SysTick->VAL;
...
int main4(void) { // main4
  ...
  while (1) {
    ...
    FloatOutFixtime =
        ((startTime - stopTime) & 0x0FFFFFF) - Offset; // in bus cycles
    stopTime2=SysTick->VAL;
    TotalTime = ((startTime2 - stopTime2) & 0x0FFFFFF) - Offset; // in bus cycles
    Clock_Delay1ms(100);
  }
}
...
int main(void){ // main5
  ...
  //TimerG12_IntArm(80000000/4000, 1); // fun
...
int main6(void) { // main6
  ...
  while (1) {
    ...
    while (LaunchPad_InS2() == 0x00040000) {
    }; // wait for release
