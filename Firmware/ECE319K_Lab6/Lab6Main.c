...
char EID1[]="sy24742";  // Sathvik's EID
char EID2[]="smw4789";  // Steven's EID
...
int main3(void){
  ...
  while(LaunchPad_InS1()==0x00040000){}; // wait for release
  while(LaunchPad_InS1()==0){};          // wait for touch
    ST7735_FillScreen(0);       // set screen to black
  for(i=0;i<SIZE;i++){
    ST7735_SetCursor(0,i);
    Dec2String(TestData[i],test); // your Lab 6
    ST7735_OutString(test);
    ST7735_SetCursor(8,i);
    printf("%li",TestData[i]);  // fixed point output
  }
  while(1){
  }
}
