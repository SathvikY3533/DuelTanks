...
#define IR_HIGH (GPIOA->DOUT31_0 |= IR) // Sets IR LED to high state (ON)
#define IR_LOW (GPIOA->DOUT31_0 &= ~IR) // Sets IR LED to low state (OFF)
...
#endif
  __asm volatile("Delay_Loop: SUBS  R0, R0, #1; \n"
                 "            BNE   Delay_Loop; \n");
}
